...
\text{minimize}  & \displaystyle \sum_{1 \le i \le K} \sum_{1 \le j \le n} b_{ij} & & & \qquad \qquad (1) \\
\text{subject to}& \displaystyle \sum_{j = 1}^K w_j c_{ij} \le c_i   &  & \qquad \qquad i = (1, \ldots, K) & \qquad \qquad (2) \\
                 & \displaystyle \sum_{i = 1}^n b_{ij} = 1     &  & \qquad \qquad j = (1, \ldots, n) & \qquad \qquad (3) \\
                 & \displaystyle b_{ij} \in \{0, 1\}           &  & \qquad \qquad i = (1, \ldots, K), j = (1, \ldots, n)  & \qquad \qquad (4)
\end{array}
\end{equation*}

The model has decision variables $b_{ij}$ to encode whether clique $j$ is assigned to batch $i$. It uses $K$ values $c_i$ $(i = 1, \ldots, K)$ to represent the capacity of each batch; in the default case of equal-sized batches, we have $c_i = \frac{N}{K}$ for each batch. The values $w_j$ $(j = 1, \ldots, n)$ encode the weight of each clique. Each clique represents a group a samples that must be assigned to the same batch in order to fulfill the must-link constraints; $n$ is the number of cliques. We have $n < N$, where $N$ is the total number of samples, whenever there are any must-link constraints. Constraint (2) realizes that the weight of each batch is not exceeded; constraint (3) realizes that each clique is assigned to exactly one batch. Because in our application, we need any assignment that fulfills the must-link requirements, the objective function (1) is constant for each possible assignment, and does not actually contribute to the algorithm. 

The optimization phase of the exchange algorithm also uses a downscaled data set where each patient rather than each sample corresponds to a unit of analysis. To obtain a reduced distance matrix that preserves all information of the original distance matrix, we sum up all pairwise distances between samples in different cliques. In the context of maximizing the diversity, this transformation sufficiently preserves the relevant information in the original distance matrix (Böcker et al., 2011). Using the initial assignment and the reduced distance matrix, we apply the same exchange algorithm as we described for the unrestricted anticlustering application. However, during the exchange process, we only exchange cliques of the same size (e.g., patients providing the same number of samples) to ensure that the cardinality constraints are respected throughout (i.e., usually equal-sized groups).

### Optimal anticlustering using must-link constraints

Describe extension of Papenberg and Klau (2021) ILP model here? It is quite straightforward; could include results of feasibility (runs up to N = 40-50)
