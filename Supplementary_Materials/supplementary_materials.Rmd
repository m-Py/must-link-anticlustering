...
To include must-link constraints with anticlustering, we adjusted both the initialization phase as well as the optimization phase of the exchange algorithm. For initialization, we recognize that we now may have to assign multiple samples simultaneously ("must-link groups") to a batch---not all samples can be considered independently of all other samples. In our application, samples belonging to the same patient were required to be assigned to the same batch. In the synthetic data set that resembles our actual application, we have `r nrow(tt)` samples from `r length(unique(tt$patientID))` unique patients. Most patients (n = `r table(table(tt$patientID))[1]`) were included with 1 sample, and the maximum number of samples per patient was `r max(table(tt$patientID))`. 

Our algorithm first we collect all samples of patients with more than one sample. It treats each group of samples that belong to the same patient as a unit. Each unit has a weight corresponding to the number of samples for this patient. The initialization step must assign these must-link groups to the $K$ batches; in our application, we required 16 batches. This assignment needs to ensure that the weight per batch does not exceed the maximum capacity of  $\frac{N}{K}$. This problem corresponds to the well-known bin packing problem, for which optimal and heuristic algorithms are available.
